...
- `Ptr<Value> val`表示该Interval对应的要分配寄存器的变量。
...
- `bool covers(Ptr<Instruction> inst);` 判断该Interval是否覆盖指令inst。

- `bool intersects(Ptr<Interval> interval);`判断该Interval和另一个interval是否有相交。

- `void union_interval(Ptr<Interval> interval);`将另一个interval和该interval合并。
