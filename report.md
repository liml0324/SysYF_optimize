...
循环表达式外提的第一步是扫描循环不变表达式。这一部分的思路与前面一些优化Pass类似。首先，循环不变式只考虑除`alloca`, `br`, `load`, `phi`之外的指令。然后，对于每一条指令，如果它的操作数全部在循环外被定值，且不是全局变量，那么它就是循环不变式。同样，如果是函数调用指令，则额外要求调用的函数是纯函数。
