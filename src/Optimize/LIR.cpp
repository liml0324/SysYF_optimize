    ...
    if (terminator->is_br()) {
        ...
        if (br->is_cond_br()){
            ...
            if (inst && inst->is_cmp()) {
                ...
                auto inst_cmp = dynamic_pointer_cast<CmpInst>(inst);
...
void LIR::split_srem(Ptr<BasicBlock> bb) {
    ...
    for (auto iter = instructions.begin();iter != instructions.end();iter++) {
        ...
        if (inst_rem->is_rem()) {
            ...
            if (op1 == op2) continue;
            if (dynamic_pointer_cast<ConstantInt>(op2) && dynamic_pointer_cast<ConstantInt>(op2)->get_value() == 1) continue;
...
void LIR::split_gep(Ptr<BasicBlock> bb) {
    auto &instructions = bb->get_instructions();
    for (auto iter = instructions.begin(); iter != instructions.end(); iter++) {
        auto inst_gep = *iter;
        if (inst_gep->is_gep()) {
            ...
            auto size = CONST_INT(inst_gep->get_type()->get_pointer_element_type()->get_size());
            auto offset = inst_gep->get_operand(offset_op_num);
            inst_gep->remove_operands(offset_op_num, offset_op_num);
            inst_gep->add_operand(CONST_INT(0));
