...
namespace SysYF{
namespace IR{
    std::string CodeGen::tmp_reg_restore(Ptr<Instruction>inst) {
        std::string code;
        for(auto pair:tmp_regs_loc){
            code += IR2asm::safe_load(Ptr<IR2asm::Reg>(new IR2asm::Reg(pair.first)),
                                      pair.second,
                                      sp_extra_ofst,
                                      long_func);
        }
    ...
    std::string CodeGen::push_tmp_instr_regs(Ptr<Instruction>inst) {
        std::string code;
        store_list.clear();
        to_store_set.clear();
        interval_set.clear();
        PtrSet<Value> to_ld_set = {};
        ...
        for (auto opr : inst->get_operands()) {
            if(dynamic_pointer_cast<Constant>(opr) ||
            dynamic_pointer_cast<BasicBlock>(opr) ||
            dynamic_pointer_cast<GlobalVariable>(opr) ||
            dynamic_pointer_cast<AllocaInst>(opr)){
                continue;
            }
        ...
        if(!inst->is_void() && !dynamic_pointer_cast<AllocaInst>(inst)){
        ...
        for(auto reg_id:store_list){
            ...
            auto loc = Ptr<IR2asm::Regbase>(new IR2asm::Regbase(IR2asm::Reg(13),cur_ofset*4+tmp_reg_base));
            tmp_regs_loc[reg_id] = loc;
            code += IR2asm::safe_store(Ptr<IR2asm::Reg>(new IR2asm::Reg(reg_id)),
                                       loc,
                                       sp_extra_ofst,
                                       long_func);
        ...
        for(auto opr:to_ld_set){
            code += IR2asm::safe_load(Ptr<IR2asm::Reg>(new IR2asm::Reg(reg_map[opr]->reg_num)),
                                          stack_map[opr],
                                          sp_extra_ofst,
                                          long_func);
        }
    ...
    std::string CodeGen::pop_tmp_instr_regs(Ptr<Instruction>inst) {
        std::string code;
        for(auto opr:to_store_set){
            code += IR2asm::safe_store(Ptr<IR2asm::Reg>(new IR2asm::Reg(reg_map[opr]->reg_num)),
                                           stack_map[opr],
                                           sp_extra_ofst,
                                           long_func);
        }
    ...
    std::string CodeGen::ld_tmp_regs(Ptr<Instruction>inst) {
        std::string code;
        if(!inst->is_alloca() && !inst->is_phi()){
            ...
            for(auto opr:inst->get_operands()){
                ...
                dynamic_pointer_cast<GlobalVariable>(opr) ||
                dynamic_pointer_cast<AllocaInst>(opr) ||
                dynamic_pointer_cast<Function>(opr)){
                    continue;
                }
            ...
            for(auto ld_reg:to_ld_set){
                code += IR2asm::safe_load(Ptr<IR2asm::Reg>(new IR2asm::Reg(ld_reg)),
                                          tmp_regs_loc[ld_reg],
                                          sp_extra_ofst,
                                          long_func);
            }
