...
namespace SysYF{
namespace IR{
    std::string CodeGen::caller_reg_store(Ptr<Function> fun,Ptr<CallInst> call){
        std::string code;
    ...
    std::string CodeGen::caller_reg_restore(Ptr<Function> fun, Ptr<CallInst> call){
        std::string code = "";
        ...
        std::string CodeGen::arg_move(Ptr<CallInst> call){
        //arg on stack in reversed sequence
        std::string regcode;
        std::string memcode;
        std::stack<Ptr<Value>> push_queue;//for sequence changing
        auto fun = dynamic_pointer_cast<Function>(call->get_operand(0));
        int i = 0;
        ...
        for(auto arg: call->get_operands()){
            if(dynamic_pointer_cast<Function>(arg))continue;
            if(i < 4){
                if(dynamic_pointer_cast<ConstantInt>(arg)){
                    regcode += IR2asm::space;
                    regcode += "ldr ";
                    regcode += IR2asm::Reg(i).get_code();
                    regcode += ", =";
                    regcode += std::to_string(dynamic_pointer_cast<ConstantInt>(arg)->get_value());
                    regcode += IR2asm::endl;
                    i++;
                    continue;
                }
                ...
                if(reg >= 0){
                    if(reg<=3){
                        regcode += IR2asm::safe_load(Ptr<IR2asm::Reg>(new IR2asm::Reg(i)), 
                                                Ptr<IR2asm::Regbase>(new IR2asm::Regbase(IR2asm::Reg(IR2asm::sp),caller_saved_pos[reg])),
                        ...
                        regcode += IR2asm::safe_load(Ptr<IR2asm::Reg>(new IR2asm::Reg(i)), 
                                                Ptr<IR2asm::Regbase>(new IR2asm::Regbase(IR2asm::Reg(IR2asm::sp),caller_saved_pos[12])),
                                                sp_extra_ofst, long_func);
                        i++;
                        continue;
                    }
        ...
        while(!push_queue.empty()){
            ...
            if(dynamic_pointer_cast<ConstantInt>(arg)){
                ...
                memcode += " ,=";
                memcode += std::to_string(dynamic_pointer_cast<ConstantInt>(arg)->get_value());
                memcode += IR2asm::endl;
                ...
                if(reg >= 0){
                    if(reg>=4&&reg<12){
                        ...
                        memcode += IR2asm::safe_load(Ptr<IR2asm::Reg>(new IR2asm::Reg(tmp_reg_id[tmp_reg_size-remained_off_reg_num])),
                                                     Ptr<IR2asm::Regbase>(new IR2asm::Regbase(IR2asm::Reg(IR2asm::sp),caller_saved_pos[reg])),
                                                     sp_extra_ofst, 
                                                     long_func);
                        to_push_regs.push_back(tmp_reg_id[tmp_reg_size-remained_off_reg_num]);
    ...
    std::string CodeGen::callee_arg_move(Ptr<Function> fun){
        ...
        std::map<int, Ptr<IR2asm::Regbase>> conflict_reg_loc;
        int size = 0;
