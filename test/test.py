...
def eval(EXE_PATH, TEST_BASE_PATH, optimization):
    ...
    for case in testcases:
        ...
        # 生成IR
        time_start_ir = time.time()
        IRBuild_result = subprocess.run(IRBuild_ptn.format(EXE_PATH, LL_PATH, SY_PATH, IR_Optimiazation), shell=True, stderr=subprocess.PIPE)
...
if __name__ == "__main__":
    ...
    # you can only modify this to add your testcase
    print("IR Optimization Level: "+IR_Optimiazation)

    optimization = "-O0"     # -O0 -O1 -O2 -O3 -O4 -Ofast
    fail_dirs = set()
    for TEST_BASE_PATH in TEST_DIRS:
        testcases = {}  # { name: need_input }
        EXE_PATH = os.path.abspath('../build/SysYFCompiler') # 编译器路径 # 编译器路径
